...
export class TokenManager {
  ...
  static hasRefreshToken(): boolean {
    ...
    // 3. 최근 로그인 기록이 있는지 확인 (2FA 상태나 사용자 캐시가 있으면 가능성 높음)
    const hasTwoFACache = localStorage.getItem('twofa_state');
    const hasUserCache = localStorage.getItem('user_state_cache');
    if (hasTwoFACache || hasUserCache) {
...
/**
 * 2FA 상태 캐시 관리 (개선된 버전)
 * - 로그인 시 2FA 상태를 로컬 스토리지에 저장
 * - 토큰 갱신 시 캐시된 상태 사용
 * - 사용자 상태 캐시와 연동하여 일관성 보장
 */
export class TwoFAStateManager {
  private static readonly CACHE_KEY = 'twofa_state';
  private static readonly MAX_AGE = 24 * 60 * 60 * 1000; // 24시간
  
  /**
   * 2FA 상태 저장 (안정성 개선)
   */
  static setTwoFAState(enabled: boolean): void {
    try {
      const data = { 
        enabled, 
        timestamp: Date.now(),
        version: '1.1' // 캐시 구조 버전
      };
      localStorage.setItem(this.CACHE_KEY, JSON.stringify(data));
      console.log('[TwoFAState] ✅ Cached 2FA state:', enabled, 'at', new Date().toISOString());
    } catch (error) {
      console.warn('[TwoFAState] ❌ Failed to cache 2FA state:', error);
    }
  }
  
  /**
   * 캐시된 2FA 상태 조회 (신뢰성 개선)
   */
  static getTwoFAState(): boolean | null {
    try {
      const cached = localStorage.getItem(this.CACHE_KEY);
      if (!cached) {
        console.log('[TwoFAState] 📭 No cached 2FA state found');
        return null;
      }
      ...
      if (typeof data === 'boolean') {
        console.log('[TwoFAState] 🔄 Converting legacy cache format');
        this.setTwoFAState(data); // 새 형식으로 저장
        return data;
      }
      ...
      // 타입 검증
      if (typeof enabled !== 'boolean' || typeof timestamp !== 'number') {
        console.warn('[TwoFAState] ⚠️ Invalid cache data format, clearing');
      ...
      if (age > this.MAX_AGE) {
        console.log('[TwoFAState] ⏰ Cached 2FA state expired (age:', Math.round(age / 1000 / 60), 'minutes)');
        this.clearTwoFAState();
        return null;
      }
      
      console.log('[TwoFAState] 📱 Using cached 2FA state:', enabled, 'age:', Math.round(age / 1000), 'seconds');
      return enabled;
    } catch (error) {
      console.warn('[TwoFAState] ❌ Failed to read cached 2FA state:', error);
      this.clearTwoFAState(); // 손상된 캐시 정리
      return null;
    }
  }
  
  /**
   * 2FA 상태 캐시 만료시간 갱신 (기존 값 유지)
   */
  static refreshTwoFAStateExpiry(): void {
    const currentState = this.getTwoFAState();
    if (currentState !== null) {
      this.setTwoFAState(currentState);
      console.log('[TwoFAState] 🔄 Refreshed 2FA cache expiry');
    }
  }
   ...
   * 2FA 상태 캐시 삭제
   */
  static clearTwoFAState(): void {
    try {
      localStorage.removeItem(this.CACHE_KEY);
      console.log('[TwoFAState] 🗑️ Cleared 2FA state cache');
    } catch (error) {
      console.warn('[TwoFAState] ❌ Failed to clear 2FA state cache:', error);
    }
  }
  
  /**
   * 2FA 상태와 사용자 상태 캐시 동기화 확인
   */
  static validateWithUserCache(): boolean {
    try {
      const twoFAState = this.getTwoFAState();
      const userCache = localStorage.getItem('user_state_cache');
      
      if (twoFAState !== null && userCache) {
        const userData = JSON.parse(userCache);
        const userTwoFA = userData.user?.twoFactorEnabled;
        
        if (userTwoFA !== undefined && userTwoFA !== twoFAState) {
          console.warn('[TwoFAState] ⚠️ 2FA cache mismatch with user cache, syncing...');
          this.setTwoFAState(userTwoFA);
          return false; // 불일치 발견
        }
      }
      
      return true; // 일치 또는 확인 불가
    } catch (error) {
      console.warn('[TwoFAState] ❌ Failed to validate with user cache:', error);
      return false;
    }
  }
}

// 디버깅을 위해 window 객체에 TwoFAStateManager 노출
if (typeof window !== 'undefined') {
  (window as any).TwoFAStateManager = TwoFAStateManager;
}
